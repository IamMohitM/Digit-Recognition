...
import os
import logging
from models import DigitEfficientNet, CNN
...
base_name = 'digit_model_mobilenet'
model_name = f"{base_name}.pth"
torch_script_model_name = f"{base_name}_torchscript.pt"
weights_name = f"{base_name}_weights.pth"
...
def validate(model: torch.nn.Module, validation_dataset):
    ...
    logger.info(f'Epoch Validation Accuracy: {acc} - Highest Accuracy : {max_val_accuracy} ')
...
def train(model: torch.nn.Module, train_dataloader: torch.utils.data.DataLoader, val_dataloader: torch.utils.data.DataLoader = None, epochs: int = 20):
    ...
    for epoch in range(1, epochs+1):
        ...
        for batch, data in enumerate(train_dataloader, 0):
            ...
            logger.info(f'Epoch:{epoch} Batch: {batch} Acc: {correct_preds/inputs.size(0)} loss: {loss.item()}')
